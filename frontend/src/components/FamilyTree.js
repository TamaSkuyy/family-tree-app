...
import { ArrowLeft, Plus, Link2, Heart, User, Users } from 'lucide-react';
...
const FamilyTree = ({ personId }) => {
    ...
    if (loading) {
        return <div className="flex justify-center items-center h-64">Loading family tree...</div>;
    }

    if (!treeData) {
        return <div className="text-center py-12 text-gray-500">No family tree data found</div>;
    }
        ...
        <div className="max-w-6xl mx-auto p-6">
            ...
            {selectedPerson && (
                <section aria-labelledby="selected-person" className="mb-6">
                    <h2 id="selected-person" className="sr-only">Selected person</h2>
                        ...
                        <div className="card-body flex items-center justify-between">
                            <div>
                                ...
                                <div className="flex gap-3 items-center text-sm text-gray-600">
                                    <span className={`badge ${selectedPerson.gender === 'male' ? 'badge-info' : 'badge-secondary'}`}>
                                        {selectedPerson.gender}
                                    </span>
                                    {selectedPerson.birth_date && (
                                        <span>Born: {new Date(selectedPerson.birth_date).toLocaleDateString()}</span>
                                    )}
                                    {selectedPerson.death_date && (
                                        <span>Died: {new Date(selectedPerson.death_date).toLocaleDateString()}</span>
                                    )}
                                </div>
                            </div>
            ...
            <div className="bg-white border rounded-lg p-6 shadow-sm">
                ...
                <section aria-labelledby="parents" className="mb-6">
                    <h4 id="parents" className="font-medium text-gray-700 mb-2 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {(!selectedPerson?.parent_relationships || selectedPerson.parent_relationships.length === 0) && (
                ...
                <section aria-labelledby="spouses" className="mb-6">
                    <h4 id="spouses" className="font-medium text-gray-700 mb-2 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {(!selectedPerson?.spouse_relationships || selectedPerson.spouse_relationships.length === 0) && (
                ...
                <section aria-labelledby="children" className="mb-6">
                    <h4 id="children" className="font-medium text-gray-700 mb-2 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        {(!selectedPerson?.child_relationships || selectedPerson.child_relationships.length === 0) && (
        ...
        >
            <div className="card-body">
                <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${person.gender === 'male' ? 'bg-blue-100' : 'bg-pink-100'}`}>
    ...
    useEffect(() => {
        const fetchAllPersons = async () => {
            try {
                ...
                setAllPersons(response.data.data.filter(p => p.id !== currentPerson.id));
            } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-2">Related Person</label>
                        <select
